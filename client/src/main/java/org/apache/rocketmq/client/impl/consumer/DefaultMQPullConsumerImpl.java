...
public class DefaultMQPullConsumerImpl implements MQConsumerInner {
    ...
    /**
     * 读取偏移量，拉取消费时通过该api读取指定队列的偏移量进行消费
     *
     * @param mq        消息队列
     * @param fromStore 从持久化文件还是缓存中去读
     * @return
    ...
    /**
     * pull模式负载均衡接口，每次拉取消息需要重新拉起topic下消息队列
     *
     * @param topic
     * @return
     * @throws MQClientException
     ...
     *
     * @param mq               消息队列
     * @param subscriptionData 订阅关系
     * @param offset           偏移量
     * @param maxNums          最大拉取数据
     * @param block            是否阻塞
     * @param timeout          超时时间
     * @return
     * @throws MQClientException
    ...
    public synchronized void start() throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                /**
                 * 创建一个MQClientInstance对象，如果MQClientInstance已经创建则返回一个缓存的MQClientInstance实例，该对象为和兴对象。每个clientId一个实例
                 */
                this.mQClientFactory = MQClientManager.getInstance().getAndCreateMQClientInstance(this.defaultMQPullConsumer, this.rpcHook);

                /**
                 * 设置Rebalance对象，消费者组、消费模式、消费queue分配算法、MQClientInstance对象
                 */
                ...
                /**
                 * 对BrokerAPI的封装类PullAPIWrapper进行初始化，同时注册消息过滤filter
                 */
    ...
    /**
     * 更新队列偏移量,更新到本地缓存，然后mqInstance定时持久化到broker端
     *
     * @param mq     mq
     * @param offset 偏移量
     * @throws MQClientException
